$(document).ready(function () {
    ...
    fetch_dep_data = tasksArr.filter((dep) => dep.dep_name === 'Full-stack');

    $('#filterStatus').change(function () {
        let filterStatus = $(this).val();

        if (filterStatus === 'Pending') {
            fetch_dep_data = tasksArr.filter((dep) => dep.status === 'Pending' && dep.dep_name === 'Full-stack');
        } else if (filterStatus === 'In-progress') {
            fetch_dep_data = tasksArr.filter((dep) => dep.status === 'In-progress' && dep.dep_name === 'Full-stack');
        } else if (filterStatus === 'Complete') { // Corrected condition
            fetch_dep_data = tasksArr.filter((dep) => dep.status === 'Complete' && dep.dep_name === 'Full-stack');
        } else {
            fetch_dep_data = tasksArr.filter((dep) => dep.dep_name === 'Full-stack');
        }
        updateHTML();
